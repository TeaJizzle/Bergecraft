...
public class Config {
  ...
  public static Config initialize(Bergecraft plugin) {
  ...
  private Bergecraft plugin_ = null;
  private Set<Integer> remove_item_drops_ = null;

  public Config(Bergecraft plugin) {
    plugin_ = plugin;
    scanAnnotations();
  }
  ...
  private void addToConfig(Bergification bug) {
    if (dynamicOptions_.containsKey(bug.opt())) {
      Bergecraft.info("Duplicate configuration option detected: " + bug.opt());
      return;
    }
  ...
  private void scanAnnotations() {
    try {
      for (Method method : Bergecraft.class.getMethods()) {
